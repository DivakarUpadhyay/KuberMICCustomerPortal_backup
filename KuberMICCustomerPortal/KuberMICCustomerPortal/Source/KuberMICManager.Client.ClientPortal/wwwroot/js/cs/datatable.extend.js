...
class DatatableExtend {
  ...
  _addListeners() {
    ...
    // Export listeners
    document.querySelectorAll('.datatable-export').forEach((el) => {
  ...
  _onExportClick(event) {
    event.preventDefault();
    try {
        this._getDatatable(event.currentTarget).buttons(1).trigger();
    } catch (error) {
        console.log('Trigger button is not found');
    }
  }
  ...
  _onLengthClick(event) {
    event.preventDefault();
    var length = parseInt(event.currentTarget.innerHTML);
    if (isNaN(length))
      length = -1;
